...
const Register = ({ handleVisibility, handleLoginVisibility }) => {
  ...
  return (
    ...
    <div className='w-full flex items-start justify-between px-5 pt-7'>
      ...
      onClick={() => { handleLoginVisibility(true); handleVisibility(false);}}>Log in</Button>
    </div>
    <div className='mt-8 w-full bg-gray-900 pb-5 flex flex-col items-center'>
      <h1 className='text-2xl font-bold'>Create an account</h1>
      <form className='mt-9 w-full md:w-auto px-8 md:px-0' onSubmit={() => handleSubmit()}>
          ...
          <div className='mt-4 w-full md:w-80 h-11 flex items-center bg-gray-800 border border-slate-600 rounded-xl px-5'>
              ...
              type='email' placeholder='E-mail' className='w-full h-full bg-transparent focus:outline-none placeholder-slate-200'
              onChange={handleChange} onBlur={handleBlur}/>
          </div>
          <div className='mt-4 w-full md:w-80 h-11 flex items-center bg-gray-800 border border-slate-600 rounded-xl px-5'>
              <input 
          ...
          <Button disabled={values.username.length === 0 || values.password.length === 0 || values.email.length === 0 || loading } type='submit' variant='loginPrimary' size='info' className='w-full md:w-80 mt-7'>Log in</Button>
      </form>
      ...
      <p className='font-bold text-lg mt-5 sm:hidden'>Already have an account? <span className='uppercase tracking-wide text-sky-400' onClick={() => { handleLoginVisibility(true); handleVisibility(false);}}>Log in</span></p>
    </div>
  </div>
  )
}
