  ...
  return (
    <div className='absolute w-screen h-screen inset-0 bg-gray-900 text-white z-50'>
      <div className='w-full flex items-start justify-between px-5 pt-7'>
        ...
        onClick={() => { handleRegisterVisibily(true); handleVisibility(true);}}>Register</Button>
      </div>
      <div className='mt-8 w-full bg-gray-900 pb-5 flex flex-col items-center'>
        <h1 className='text-2xl font-bold'>Log in</h1>
        <form className='mt-9' onSubmit={() => handleSubmit(false)}>
            <div className='w-80 h-11 flex items-center bg-gray-800 border border-slate-600 rounded-xl px-5'>
                <input 
                value={values.username} name='username'
                type='text' placeholder='Username or e-mail' className='w-full h-full bg-transparent focus:outline-none'
                onChange={handleChange} onBlur={handleBlur}/>
            </div>
            <div className='mt-4 w-80 h-11 flex items-center bg-gray-800 border border-slate-600 rounded-xl px-5'>
                <input 
                value={values.password} name='password'
                type='password' placeholder='Password' className='w-9/12 h-full bg-transparent focus:outline-none'
                onChange={handleChange} onBlur={handleBlur}/>
            ...
            <Button disabled={values.username.length === 0 || values.password.length === 0 || loading } type='submit' variant='loginPrimary' size='info' className='w-80 mt-7'>Log in</Button>
        </form>
        <Button disabled={loading} type='button' variant='loginPrimary' size='info' className='w-80 mt-3'
        onClick={() => handleBlur(true)}>Log in as a guest</Button>
        <div className='relative mt-7 w-80 flex justify-center items-center'>
            <span className='absolute -top-2  bg-gray-900 text-slate-600 font-bold uppercase tracking-wide w-10 text-center'>Or</span>
            <div className='h-1 w-80 border-t-2 border-slate-600 mt-1'/>
        </div>
        <div className='mt-7 flex items-center space-x-4'>
           <Button type='button' variant='loginTransparent' size='info' className='w-36'>
        ...
        <p className='font-bold text-lg mt-5 sm:hidden'>Don't have an account? <span className='uppercase tracking-wide text-sky-400'>Register</span></p>
      </div>
    </div>
  )
}
